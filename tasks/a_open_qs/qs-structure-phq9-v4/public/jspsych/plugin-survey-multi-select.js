var jsPsychSurveyMultiSelect = (function (jspsych) {
    ...
    class SurveyMultiSelectPlugin {
        ...
        trial(display_element, trial) {
            ...
            trial_form.innerHTML += alert_content_oq
            ...
            trial_form.addEventListener("submit", (event) => {
                ...
                // Only works for one (last) question
                let is_empty = question_data[name].length === 0;
                var trial_data = {
                    rt: response_time,
                ...
                /***
                 * Added by Jakub Onysk 20/09/2024
                 */
            ...
            if (trial.trial_duration !== null) {
                this.jsPsych.pluginAPI.setTimeout(() => {
                    ...
                    let question_data = {};
                    let match = display_element.querySelector("#jspsych-survey-multi-select-" + 0);
                    let val = [];
                    let inputboxes = match.querySelectorAll("input[type=checkbox]:checked");
                    for (var j2 = 0; j2 < inputboxes.length; j2++) {
                        let currentChecked = inputboxes[j2];
                        val.push(currentChecked.value);
                    }
                    let id = "Q" + 0;
                    var obje = {};
                    ...
                    // works only for one (last) question
                    let is_empty = question_data[name].length === 0;
