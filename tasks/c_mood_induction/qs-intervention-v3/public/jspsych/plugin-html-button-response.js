var jsPsychHtmlButtonResponse = (function (jspsych) {
    ...
    parameters: {
      ...
      button_html: {
        ...
        default: function(choice, choice_index) {
          return `<button class="jspsych-btn" id="jspsych-btn_mine_${choice_index}">${choice}</button>`;
        }
      },
      prompt: {
        type: jspsych.ParameterType.HTML_STRING,
  ...
  class HtmlButtonResponsePlugin {
    ...
    trial(display_element, trial) {
      ...
      stimulusElement.innerHTML += `<div id="dim-overlay-opt"></div>`
      if (trial.do_countdown) {
        stimulusElement.innerHTML += timer_code
      }
      ...
      let custom_instr_el = document.getElementsByClassName('pre_trial_instr')
      // console.log(custom_instr_el)
      if (custom_instr_el && custom_instr_el.length > 0){
        custom_instr_el = custom_instr_el[0]
        custom_instr_el.appendChild(buttonGroupElement)
      }else{
        stimulusElement.appendChild(buttonGroupElement);
      }
      var start_time = performance.now();
      timerControl.trialStarted = start_time
      var response = {
        rt: null,
        button: null
      };
