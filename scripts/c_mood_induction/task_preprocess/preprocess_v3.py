...
task_version = 'qs-intervention-v3'
task_subversion = ''
...
feedback_cols = [f"response_closed_q_feedback_{i}" for i in range(6)]
feedback_rename_cols = ['shoes', 'mood_change_rec', 'sim_sit', 'mood_change_new', 'demand', 'mood_change_new2']
feedback_rename = {o: n for o, n in zip(feedback_cols, feedback_rename_cols)}
feedback_cols = common_cols + feedback_cols

baseline_vas_mood_cols = ["response_vas_mood_baseline_0"]
b_vas_mood_rename = {o: n for o, n in zip(baseline_vas_mood_cols, ["vas_mood_baseline"])}
baseline_vas_mood_cols = common_cols + baseline_vas_mood_cols

fu_vas_mood_cols = ["response_vas_mood_fu_0"]
fu_vas_mood_rename = {o: n for o, n in zip(fu_vas_mood_cols, ["vas_mood_fu"])}
fu_vas_mood_cols = common_cols + fu_vas_mood_cols
...
if len(duplicates) > 0:
    ...
    good_idx = (sub_data['consented'] == 'Yes') & (sub_data['completed'] == 'Yes') & (sub_data['returned'] == 'No') & (
            sub_data['code'] == completion_code) & (sub_data['attention_checks_bool'] == False) & (
    ...
    sub_ts_ids = (sub_data['date_time'].astype(int) // 10 ** 9).astype(str) + '_' + sub_data['PID'].str[-5:].astype(str)
    # sub_ts_ids = (sub_data['date_time'].values.astype(int) // 10 ** 9).astype(str) + '_' + sub_data['PID'].values.astype(str).str[-5:]
    ...
    sub_ts_ids = (baseline_data['date_time'].astype(int) // 10 ** 9).astype(str) + '_' + baseline_data['PID'].str[-5:]
    ...
    fu_data = fu_data[fu_data['PID'].isin(good_pid)].sort_values(['sub_ts_id'], ascending=True).reset_index(drop=True)
    ...
    sub_ts_ids = (feedback_data['date_time'].astype(int) // 10 ** 9).astype(str) + '_' + feedback_data['PID'].str[-5:]
    ...
    sub_ts_ids = (recreate_data['date_time'].astype(int) // 10 ** 9).astype(str) + '_' + recreate_data['PID'].str[-5:]
    ...
    phq9_data_f = fu_data[fu_phq9_cols].rename(columns=f_phq9_rename).melt(id_vars=common_cols, value_vars=phq9_cols,
                                                                           value_name='score_fu', var_name='question')
    ...
    # MOOD
    vas_mood_data_b = baseline_data[baseline_vas_mood_cols].rename(columns=b_vas_mood_rename).melt(id_vars=common_cols,
                                                                                                   value_vars=list(
                                                                                                       b_vas_mood_rename.values()),
                                                                                                   value_name='mood_b',
                                                                                                   var_name='question')

    vas_mood_data_f = fu_data[fu_vas_mood_cols].rename(columns=fu_vas_mood_rename).melt(id_vars=common_cols,
                                                                                        value_vars=list(
                                                                                            fu_vas_mood_rename.values()),
                                                                                        value_name='mood_fu',
                                                                                        var_name='question')
    vas_mood_data = vas_mood_data_b
    vas_mood_data.insert(vas_mood_data.shape[-1], 'mood_fu', vas_mood_data_f['mood_fu'])
    # vas_mood_data = pd.merge(vas_mood_data_b, vas_mood_data_f,
    #                          on=['sub', 'task_version', 'condition', 'group', 'question'])
    vas_mood_data.insert(vas_mood_data.shape[-1], 'mood_diff', vas_mood_data['mood_fu'] - vas_mood_data['mood_b'])
    vas_mood_data.insert(vas_mood_data.shape[-1], 'mood_diff_rel',
                         (vas_mood_data['mood_fu'] - vas_mood_data['mood_b']) / (vas_mood_data['mood_b'] + 1e-5))

    vas_mood_data['sub'] = vas_mood_data['sub'] + '_' + vas_mood_data['task_version'].str.replace('qs-intervention-',
                                                                                                  '')
    ...
    reordered_cols = [c for c in openq_data.columns if '_rt' not in c] + [c for c in openq_data.columns if '_rt' in c]
    openq_data = openq_data[reordered_cols]

    # feedback data
    feedback_data = feedback_data[feedback_cols].rename(columns=feedback_rename)
    feedback_data['sub'] = feedback_data['sub'] + '_' + feedback_data['task_version'].str.replace('qs-intervention-',
                                                                                                  '')
    ...
    if save_external:
        ...
        vas_mood_data.to_csv(f"{data_path}/processed/mood_data.csv", index=False)
    ...
    if local_path != data_path:
        ...
        vas_mood_data.to_csv(f"{local_path}/processed/mood_data.csv", index=False)
