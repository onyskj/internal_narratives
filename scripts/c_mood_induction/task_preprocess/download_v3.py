...
# %%
# feedback columns
feedback_vas_cols = [f"response_closed_q_feedback_{i}" for i in range(6)]
...
baseline_vas_mood_cols = ["response_vas_mood_baseline_0", "rt_vas_mood_baseline_0"]
...
baseline_cols = baseline_phq9_cols + baseline_vas_mood_cols + baseline_mood_cols + baseline_energy_cols + baseline_recall_cols
...
fu_phq9_cols = [f"response_phq9_fu_{i}" for i in range(10) if i != f_phq9_acheck_i] + [f"rt_phq9_fu_{i}" for i in
                                                                                       range(10) if
                                                                                       i != f_phq9_acheck_i]
fu_vas_mood_cols = ["response_vas_mood_fu_0", "rt_vas_mood_fu_0"]
fu_recall_cols = ['responses_recall3', 'rts_recall3', 'timeout_recall3']

fu_cols = fu_phq9_cols + fu_vas_mood_cols + fu_oq_cols + fu_recall_cols
...
if len(duplicates) > 0:
    ...
    for sub in sub_ids:
        ...
        sub_baseline_phq9 = db.collection('subjects').document(sub).collection('baseline').document(
            'phq9').get().to_dict()
        sub_baseline_vas_mood = db.collection('subjects').document(sub).collection('baseline').document(
            'vas_mood').get().to_dict()
        # sub_baseline_oq = db.collection('subjects').document(sub).collection('baseline').document(
        #     'open_q').get().to_dict()
        ...
        for baseline_vas_mood_key in baseline_vas_mood_cols:
            if baseline_vas_mood_key not in sub_baseline_vas_mood.keys():
                sub_baseline_vas_mood[baseline_vas_mood_key] = np.nan
                print('\t\tmissing: ' + baseline_vas_mood_key)
        ...
        sub_baseline = sub_info | sub_baseline_phq9 | sub_baseline_vas_mood | sub_baseline_mood | sub_baseline_energy | sub_baseline_recall
        baseline_rows.append(sub_baseline)

        # fu <===========
        sub_fu_phq9 = db.collection('subjects').document(sub).collection('fu').document('phq9').get().to_dict()
        sub_fu_vas_mood = db.collection('subjects').document(sub).collection('fu').document('vas_mood').get().to_dict()
        ...
        for fu_vas_mood_key in fu_vas_mood_cols:
            if fu_vas_mood_key not in sub_fu_vas_mood.keys():
                sub_fu_vas_mood[fu_vas_mood_key] = np.nan
                print('\t\tmissing: ' + fu_vas_mood_key)
        ...
        sub_fu = sub_info | sub_fu_phq9 | sub_fu_vas_mood | sub_fu_oq_pospert | sub_fu_recall
